...
public class Maze {
	...
	* It contains a pointer to the dorid and current coordinates of the droid in the maze.
	* 
	* @package mazePD
	* @author(David North) 
	*/
	...
	*/
	...
	private void createMaze(int level)
	{
		...
		if (level < getMazeDepth()-1 ) maze[portalX][portalY][level].setCellContent(Content.PORTAL_DN);
			else maze[portalX][portalY][level].setCellContent(Content.END);
	...
	private Cell getStart(int level)
	{
		...
		if (level < getMazeDepth()-1) maze[startCell.getLocX()][startCell.getLocY()][level==-1 ? 0:level+1].setCellContent(Content.PORTAL_UP);
	...
	*/	
	public Coordinates move(DroidInterface droid, Direction direction )
	{
		...
		if ((direction == Direction.DN && currentCell.getCellContent() != Content.PORTAL_DN) ||
				(direction == Direction.UP && currentCell.getCellContent() != Content.PORTAL_UP) ) return currentCell.getCoordinates();
        ...
        else return currentCell.getCoordinates();
	}
	...
	*/
	public Coordinates usePortal(DroidInterface droid, Direction direction)
	{
		Cell currentCell = getCellForDroid(droid);
			...
			{   if (currentCell.getLocZ() < getMazeDim()-1)
				{
					Coordinates newCoord = new Coordinates(currentCell.getLocX(),currentCell.getLocY(),currentCell.getLocZ()+1);
	...
	{
		if (moveToCell.isGenVisit()) return false;
	...
	*/
	...
	/** 
	 * I, Lana, created this one function to facilitate the display of the maze in the function. 
	 * Gets the content of a specific coordinate in the maze.
	 * 
	 * @param c - the coordinate to get the content for
	 * @return the contents of the cell at that coordinate
	 */
	public Content getContentForCoordinates(Coordinates c) {
		try {
		
			return this.maze[c.getX()][c.getY()][c.getZ()].getCellContent();
